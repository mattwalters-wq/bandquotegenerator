    ...
    <div ref={ref} style={{ background: "#1a1a2e", color: "#fff", fontFamily: "'Helvetica Neue', Helvetica, Arial, sans-serif", minHeight: 700, position: "relative", overflow: "hidden", width: 580 }}>
      ...
      <div style={{ padding: "28px 32px 40px" }}>
        ...
        {form.shows.map((show, idx) => (
          <div key={idx}>
            {multiShow && (
              <p style={{ color: "#e94560", fontSize: 11, fontWeight: 700, letterSpacing: 1, margin: idx === 0 ? "0 0 6px" : "14px 0 6px", textTransform: "uppercase" }}>
                {show.activity} {idx + 1} OF {form.shows.length}
              </p>
            )}
            {!multiShow && <SectionTitle>ENGAGEMENT DETAILS</SectionTitle>}
            <div style={{ display: "grid", gridTemplateColumns: "150px 1fr", gap: "5px 12px", marginBottom: multiShow ? 8 : 20 }}>
            ...
            {multiShow && idx < form.shows.length - 1 && <div style={{ height: 1, background: "#262650", margin: "4px 0" }} />}
          </div>
        ))}
        ...
        <div style={{ marginBottom: 20 }}>
          <div style={{ display: "grid", gridTemplateColumns: "1fr 70px 36px 80px", background: "#0f3460", padding: "7px 10px", borderRadius: "4px 4px 0 0", fontSize: 11, fontWeight: 700, letterSpacing: 0.5 }}>
            <span>ITEM</span><span>AMOUNT</span><span>QTY</span><span>TOTAL</span>
          </div>
          {rows.map((r, i) => (
            <div key={i} style={{ display: "grid", gridTemplateColumns: "1fr 70px 36px 80px", padding: "6px 10px", fontSize: 11, background: i % 2 === 1 ? "#16213e" : "transparent" }}>
              <span>{r.item}</span><span>{formatCurrency(r.amount)}</span><span>{r.qty}</span><span>{formatCurrency(r.total)}</span>
            </div>
          ))}
          <div style={{ display: "grid", gridTemplateColumns: "1fr 70px 36px 80px", padding: "8px 10px", background: "#e94560", borderRadius: "0 0 4px 4px", fontWeight: 700, fontSize: 13 }}>
        ...
        <CT>Upon acceptance of this offer, a tour schedule and charts will be prepared and distributed.</CT>
        <CT>Fees are payable within 14 days of the invoice date, following the performance.</CT>
        <CT>Superannuation contributions will be made at {form.superRate}% of the show fee to the nominated fund.</CT>
        <CT>Please add the living allowance (per diem) to your invoice.</CT>
