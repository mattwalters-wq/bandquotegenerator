...
//
// Ground transport is NOT included in `total`. Per the clarified travel policy
// it is a reimbursement of UP TO $transportCap, on receipts, "agreed in the
// booking offer, not added afterwards". It is exposed separately as
// `transportCap` so the UI/email can show it as its own agreed line rather than
// silently baking a flat $80 into every quote.
export function computeMusician({ feeKey, travelDays = 0, nights = 0, gstRegistered = false }) {
  ...
  const total = showFee + travel + perDiem + superAmt + gst;
  return {
    showFee,
    travelDays,
    travel,
    ...
    super: superAmt,
    gst,
    transportCap: POLICY.transportCap, // separate agreed line, NOT in `total`
    total,
    lines: [
