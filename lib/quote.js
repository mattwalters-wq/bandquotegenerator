...
export function quoteEmail(snapshot) {
  ...
  if (selected.musicians > 0) {
    ...
    if (selected.per.transportCap) {
      body += "  - Plus ground transport: up to " + money(selected.per.transportCap) + " per musician, agreed in the offer, on receipts (not in the totals above)\n";
    }
  }
  ...
  body += "\nThese figures are the band fees each musician can invoice for this trip. Invoices that match the offer are paid within 14 days.";
  body += "\nGround transport of up to " + money(POLICY.transportCap) + " per musician is agreed in the booking offer and reimbursed on receipts - it is not added afterwards.";
