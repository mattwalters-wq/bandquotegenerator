...
import { getArtist, lineupsFor, DEFAULT_ARTIST } from "./artists";
...
export function compareLineups(trip, artist) {
  let prev = 0;
  return lineupsFor(artist || getArtist(DEFAULT_ARTIST)).map((l) => {
...
export function selectedBand(trip, artist) {
  const ls = lineupsFor(artist || getArtist(DEFAULT_ARTIST));
...
export function buildQuoteSnapshot(input) {
  ...
  return {
    version: 3,
    artist: { key: artist.key, name: artist.name, shortName: artist.shortName, artistFee: artist.artistFee },
    createdRates: {
...
export function snapshotArtist(snapshot) {
  ...
  return { key: emma.key, name: emma.name, shortName: emma.shortName, artistFee: snapshot?.emmaFeeReference ?? emma.artistFee };
}
