...
export function buildRows(form) {
  ...
  if (form.hasTravelDay) {
    const baseFee = Number(form.shows[0]?.performanceFee || DEFAULT_VALUES.interstateFee);
    const td = Math.round(baseFee * 0.5);
    const travelDays = Number(form.travelDays) || 1;
    rows.push({ item: "Travel day (50% of show fee)", amount: td, qty: travelDays, total: td * travelDays });
    total += td * travelDays;
  }
