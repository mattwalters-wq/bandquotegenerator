...
export const EMPTY_SHOW = {
  ...
  rehearsalHours: "",
  rehearsalFee: 0,
  // Optional flat petrol/travel allowance for this engagement (e.g. Sarah's
  // band: Jesse gets $50 petrol per show and per rehearsal). Not super-able.
  petrolFee: 0,
};
...
export function buildRows(form) {
  ...
  // Off Day fees, MD fees, petrol and per diems never enter this base.
  let superBase = 0;
  // "additional" (Emma: super added on top) or "inclusive" (Sarah: baked in).
  const superMode = (getArtist(form.artist).policy || {}).superMode || "additional";
  ...
  form.shows.forEach((show, i) => {
    ...
    // Optional flat petrol allowance for this engagement (an expense - never
    // part of the super base).
    if (Number(show.petrolFee) > 0) {
      const pf = Number(show.petrolFee);
      rows.push({ item: "Petrol allowance" + (multiShow ? " - " + ref : ""), amount: pf, qty: 1, total: pf });
      total += pf;
    }
  });
  ...
  // Inclusive mode: super is already inside the fees, so nothing is added on
  // top - superAmount stays 0 and the conditions note says "inclusive".
  const superAmount = superMode === "inclusive" ? 0 : Math.round(superBase * superRate);

  return { rows, total, superBase, superAmount, superMode };
}
